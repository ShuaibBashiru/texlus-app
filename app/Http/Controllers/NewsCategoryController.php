...
class NewsCategoryController extends Controller
{
    ...
    public function list(){
        return view('apps.news_category.record');
    }

    public function checkIfIdIsValid($data){
        $query = NewsCategory::where('deleted_status', '=', '0')
    ...
    public function record($id)
    {
        ...
        $returnData = '';
        $result = [];
        $query = NewsCategory::from('news_category as t1')
    ...
    public function create(Request $request){
       $request->validate([
            'category_name' => 'required|unique:news_category|max:100|min:2',
        ]);
        try {
            $exist = false;
            $failed = false;
            ...
            $record = [
                ...
                $query = NewsCategory::insert($record);
                if ($query) {
                $successful = true;
                }else{
                $failed = true;
...
try {
    ...
    $newid = $id;
    $update = NewsCategory::where('generated_id', '=', $newid)
...
// End loop
    ...
    try {
        ...
        $update = NewsCategory::where('generated_id', '=', $id)
                            ->update($row);
...
$query = NewsCategory::where('generated_id', '=', $id)
