...
class PostCategoryController extends Controller
{
    ...
    public function list(){
        return view('apps.post_category.record');
    }

    public function checkIfIdIsValid($data){
        $query = PostCategory::where('deleted_status', '=', '0')
    ...
    public function record($id)
    {
        ...
        $returnData = '';
        $result = [];
        $query = PostCategory::from('post_category as t1')
                    ->where('t1.deleted_status', '=', 0)
                    ->where('t1.generated_id', $id)
                    ->leftJoin('statuses as t2', 't2.id', '=', 't1.status_id')
                    ->get(['t1.category_name', 't1.generated_id', 't1.updated_at'])->first();
    ...
    public function listAll()
    {
        ...
        $query = PostCategory::from('post_category as t1')
                    ...
                    ->get(['t1.category_name', 't1.id', 't1.descriptions', 't1.date_created',
    ...
    public function create(Request $request){
       $request->validate([
            'category_name' => 'required|unique:post_category|max:100|min:2',
        ]);
        try {
            $exist = false;
            $failed = false;
            ...
            $record = [
                ...
                "category_name" => ucfirst(strtolower($request->input('category_name'))),
                ...
                $query = PostCategory::insert($record);
                if ($query) {
                $successful = true;
                }else{
                $failed = true;
...
try {
    ...
    $newid = $id;
    $update = PostCategory::where('generated_id', '=', $newid)
...
// End loop
    ...
    try {
        ...
        $update = PostCategory::where('generated_id', '=', $id)
                            ->update($row);
...
$query = PostCategory::where('generated_id', '=', $id)
...
public function trash(Request $request)
    ...
    try {
        ...
        $row = [
            ...
            "deleted_status" => 1,
            "category_name" => 'deleted::'.$request->input('title'),
        ];
