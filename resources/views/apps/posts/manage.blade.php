   ...
   <!-- Please note that, these are just default components, 
    ...
    <div class="container-fluid">
        ...
        <!-- Body component(s) -->
           ...
           <manage_post :server_message="{{ session('message') ?? $message ?? json_encode('') }}" :server_record="{{ session('record') ?? $record ?? json_encode('') }}"></manage_post>
           <delete_post :server_record="{{ session('record') ?? $record ?? json_encode('') }}"> </delete_post>
    </div>
