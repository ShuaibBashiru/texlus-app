...
<div class="container-fluid">
    ...
    <a href="#" id="topLink"></a>
        <edit-post-category :recordId="edit_id" :timeInterval="timeInterval" />
        <div class="row">
        <div class="col-md-5">
        <div class="mt-1 mb-2">
...
export default {
    ...
    manageStatus: function(){
        ...
        $(".toaster").toast('hide')
        this.showOverlay=true;
        axios.get('/newscategory/status/updatebatch', {params:this.parameters}).then(response => {
            this.button=this.btntxt;
            this.showOverlay=false;
            this.errors = '';
             if((response.status != undefined && response.status==200) && (response['data'].data.status==response['data'].data.statusmsg)){
    ...
    swal({
       ...
       trashItem: function(id, title){
            ...
            axios.delete('/newscategory/trash', {params:this.parameters}).then(response => {
            this.button=this.btntxt;
            this.showOverlay=false;
            this.errors = '';
           if((response.status != undefined && response.status==200) && (response['data'].data.status==response['data'].data.statusmsg)){
