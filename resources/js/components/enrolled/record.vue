...
<div class="border p-2 rounded">
    ...
    <table class="table table-hover table-bordered" id="listTable">
        ...
        <tr>
            ...
            <th scope="col" class="text-truncate" @click="sortOrder('order_by')" title="">Name <i class="bi bi-sort-down float-end"></i></th>
            <th scope="col" class="text-truncate" @click="sortOrder('item_title')" title="">Course<i class="bi bi-sort-down float-end"></i></th>
            <th scope="col" class="text-truncate" @click="sortOrder('date_created')" title="">Date<i class="bi bi-sort-down float-end"></i></th>
            <th scope="col" class="text-truncate" @click="sortOrder('link_target')" title="">Progress<i class="bi bi-sort-down float-end"></i></th>
        ...
        <tr v-for="(d, index) in info.slice(startNumber, endNumber)" :key="index">
            ...
            <td class="" v-html="d.order_by ? d.order_by : ''"></td>
            <td class="" v-html="d.item_title ? d.item_title : ''"></td>
            <td class="" v-html="d.date_created ? d.date_created : ''"></td>
            <td class="" v-html="d.progress ? d.progress : '50%'"></td>
...
resetForm: function(){
   ...
   validateForm: function(){
    ...
    getRecords: function(){
        ...
        this.showOverlay=true;
        this.search = ''
        axios.get('/enrolled/list', {params:this.parameters}).then(response => {
            this.button=this.btntxt;
            this.showOverlay=false;
            this.errors = '';
            if((response.status != undefined && response.status==200) && (response['data'].data.status==response['data'].data.statusmsg)){
    ...
    manageStatus: function(){
        ...
        $(".toaster").toast('hide')
        this.showOverlay=true;
        axios.get('/enrolled/status/updatebatch', {params:this.parameters}).then(response => {
            this.button=this.btntxt;
            this.showOverlay=false;
            this.errors = '';
             if((response.status != undefined && response.status==200) && (response['data'].data.status==response['data'].data.statusmsg)){
