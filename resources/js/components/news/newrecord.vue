...
<div class="modal-dialog modal-dialog-scrollable modal-lg  ps-md-3 pe-md-3">
    ...
    <div class="row">
        ...
        <div class="mt-1 mb-2">
             <h5 class="mt-2 ms-0 text-primary"> <span v-text="pageName" onclick="history.back()" role="button"></span>  <span class="text-muted"> > New</span> </h5>
        </div>
        </div>
...
export default {
    ...
    generateLink: function(){
        ...
        let url = hostname+'/news/'+category_name+'/'+title;
   ...
   validatePost: function(){
        ...
        if(this.parameters.title=="" || this.parameters.category_id=="") {
            ...
            return false;
        }else if(this.parameters.display_media_status==1 && this.selectedFile=="") {
            this.validated = false
            this.disabled = false;
    ...
    sendPost: function(){
        ...
        axios.post('/news/create', form).then(response => {
            this.button=this.btntxt;
    ...
    getCategory: function(){
        $(".toaster").toast('hide')
        this.showOverlay=true;
        axios.get('/newscategory/list', {params:this.parameters}).then(response => {
            this.button=this.btntxt;
            this.showOverlay=false;
            this.errors = '';
