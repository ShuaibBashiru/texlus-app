...
<div class="border p-2 rounded">
    ...
    <table class="table table-hover table-bordered" id="listTable">
        ...
        <tr v-for="(d, index) in info.slice(startNumber, endNumber)" :key="index">
            ...
            <td class=""><span class="" v-text="d.read_more_status==1? 'Yes' : 'No'"></span></td>
            ...
            <td class="text-center">
            <a class="link btn btn-sm btn-warning" :href="'edit/'+encodeData(d.generated_id)"> <span class="bi bi-pencil-square"></span></a>
            </td>
        </tr>
...
export default {
    ...
    getRecords: function(){
        ...
        this.search = ''
        axios.get('/course/list', {params:this.parameters}).then(response => {
    ...
    manageStatus: function(){
        ...
        $(".toaster").toast('hide')
        this.showOverlay=true;
        axios.get('/course/status/updatebatch', {params:this.parameters}).then(response => {
            this.button=this.btntxt;
            this.showOverlay=false;
            this.errors = '';
             if((response.status != undefined && response.status==200) && (response['data'].data.status==response['data'].data.statusmsg)){
