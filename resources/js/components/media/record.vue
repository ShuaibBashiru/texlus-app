...
<div class="container-fluid">
    ...
    <a href="#" id="topLink"></a>
        ...
        <button type="button" class="btn btn-outline-primary" onclick="window.location='create'">New <i class="bi bi-plus-circle"></i></button>
...
<div class="border p-2 rounded">
    ...
    <div class="row">
        <div class="col-12">
            <hr class="mt-0"/>
            <div class="row">
                <div class="col-md-4 mt-2" v-for="(d, index) in info.slice(startNumber, endNumber)" :key="index">
                <div class="card m-2">
                    
                <img v-if="d.media_type=='Image'" :src="d.media_link ? d.media_link:nomedia" :id="'textToCopy'+index" class="card-img-top w-100" height="200" alt="No media">
                <video v-else :src="d.media_link ? d.media_link:nomedia" controls="true" class="card-img-top w-100" :id="'textToCopy'+index" height="200" alt="No media"></video>
                
                <div class="card-body border-top">
                    <h6 class="card-title text-muted" v-text="d.media_title? d.media_title:'No title'"></h6>
                    <p class="mt-3"> 
                        <small class="float-start">Width: <span v-text="d.media_width"></span><span v-text="d.media_measure"></span></small> 
                        <small class="float-end">Height: <span v-text="d.media_height"></span></small> 
                    </p>
                    <button class="btn btn-outline-primary mt-3 btn-sm w-100" @click="copyToClipboard($event)" :id="index">Copy link</button>
                </div>
                <div class="card-footer border-top">
                    <button class="btn btn-sm btn-danger float-start m-2 ms-0" @click="confirmAction(encodeData(d.generated_id), d.category_name)">Delete</button>
                    <a :href="'edit/'+encodeData(d.generated_id)" class="btn btn-sm btn-primary float-end m-2 me-0">Edit</a>
                </div>
                </div>
                </div>
...
export default {
    ...
    data (){
        ...
        checks: [],
        statuses: [],
        nomedia: '/assets/icons/nomedia.png',
        parameters:{
            selectedList: [],
            status: '',
            id: '',
    ...
    onSearch: function(){
       ...
       for (let i = 0; i < this.totalRecord.length; i++) {
           if (
            this.totalRecord[i].media_title.toString().toLowerCase().search(this.search.toString().toLowerCase())!=-1 ||
    ...
    copyToClipboard: function(e){
        var id = e.target.id;
        var textToCopy = document.getElementById('textToCopy'+id).src;
        navigator.clipboard.writeText(textToCopy)
        this.alertMsg='Copied successfully to clipboard.<br>'+textToCopy;
    ...
    getRecords: function(){
        ...
        this.search = ''
        axios.get('/media/list', {params:this.parameters}).then(response => {
    ...
    manageStatus: function(){
        ...
        $(".toaster").toast('hide')
        this.showOverlay=true;
        axios.get('/media/status/updatebatch', {params:this.parameters}).then(response => {
            this.button=this.btntxt;
            this.showOverlay=false;
            this.errors = '';
             if((response.status != undefined && response.status==200) && (response['data'].data.status==response['data'].data.statusmsg)){
    ...
    swal({
       ...
       trashItem: function(id, title){
            ...
            axios.delete('/media/trash', {params:this.parameters}).then(response => {
            this.button=this.btntxt;
            this.showOverlay=false;
            this.errors = '';
           if((response.status != undefined && response.status==200) && (response['data'].data.status==response['data'].data.statusmsg)){
