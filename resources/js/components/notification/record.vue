...
<div class="container-fluid">
    ...
    <a href="#" id="topLink"></a>
        <notification-edit :recordId="edit_id" :timeInterval="timeInterval" />
        <div class="row">
        <div class="col-md-5">
        <div class="mt-1 mb-2">
...
<div class="border p-2 rounded">
    ...
    <table class="table table-hover table-bordered" id="listTable">
        ...
        <tr>
            ...
            <th scope="col" class="text-truncate" @click="sortOrder('descriptions')" title="">Message <i class="bi bi-sort-down float-end"></i></th>
            <th scope="col" class="text-truncate" @click="sortOrder('recipient')" title="">Recipient <i class="bi bi-sort-down float-end"></i></th>
        ...
        <tr v-for="(d, index) in info.slice(startNumber, endNumber)" :key="index">
            ...
            <td class="">
                <span v-html="d.descriptions ? d.descriptions : ''"></span> <br/>
                <em class="mt-2 text-muted"><small class="float-start" v-html="d.date_created ? 'Created: '+d.date_created : ''"></small>  <small class="float-end" v-html="d.expire_date ? 'Exp: '+d.expire_date : ''"></small></em>
            </td>
            <td class="" v-html="d.recipient ? d.recipient : ''"></td>
            ...
            <td class="col-1 text-center"><button class="btn btn-sm btn-danger" @click="confirmAction(encodeData(d.generated_id), d.title)"> <i class="bi bi-trash"></i> </button></td>
...
export default {
    ...
    onSearch: function(){
       ...
       for (let i = 0; i < this.totalRecord.length; i++) {
           if (
            this.totalRecord[i].title.toString().toLowerCase().search(this.search.toString().toLowerCase())!=-1 ||
            this.totalRecord[i].descriptions.toString().toLowerCase().search(this.search.toString().toLowerCase())!=-1 ||
            this.totalRecord[i].recipient.toString().toLowerCase().search(this.search.toString().toLowerCase())!=-1 ||
    ...
    getRecords: function(){
        ...
        this.search = ''
        axios.get('/notification/list', {params:this.parameters}).then(response => {
    ...
    manageStatus: function(){
        ...
        $(".toaster").toast('hide')
        this.showOverlay=true;
        axios.get('/notification/status/updatebatch', {params:this.parameters}).then(response => {
            this.button=this.btntxt;
            this.showOverlay=false;
            this.errors = '';
             if((response.status != undefined && response.status==200) && (response['data'].data.status==response['data'].data.statusmsg)){
    ...
    swal({
       ...
       trashItem: function(id, title){
            ...
            axios.delete('/notification/trash', {params:this.parameters}).then(response => {
            this.button=this.btntxt;
            this.showOverlay=false;
            this.errors = '';
           if((response.status != undefined && response.status==200) && (response['data'].data.status==response['data'].data.statusmsg)){
