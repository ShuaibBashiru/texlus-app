...
<div class="modal fade" id="editCategoryModal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="editRoleModalLabel" aria-hiden="true">
  ...
  checkIfChanged: function(){
   ...
   validateForm: function(){
    ...
    sendPost: function(){
        ...
        axios.post('/postcategory/update', form).then(response => {
                this.button=this.btntxt;
    ...
    if (this.parameters.generated_id != ''){
        $(".toaster").toast('hide')
        this.showOverlay=true;
        axios.get('/postcategory/record/'+this.parameters.generated_id).then(response => {
            this.button=this.btntxt;
            this.showOverlay=false;
            this.errors = '';
