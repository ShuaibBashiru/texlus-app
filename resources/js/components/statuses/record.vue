...
<div class="border p-2 rounded">
    ...
    <table class="table table-hover table-bordered" id="listTable">
        ...
        <tr>
            ...
            <th scope="col" class="text-truncate" @click="sortOrder('status_name')" title="">Name <i class="bi bi-sort-down float-end"></i></th>
            <th scope="col" class="text-truncate" @click="sortOrder('status_description')" title=""> Description <i class="bi bi-sort-down float-end"></i></th>
        ...
        <tr v-for="(d, index) in info.slice(startNumber, endNumber)" :key="index">
            ...
            <td class="" v-html="d.status_name ? d.status_name : ''"></td>
            <td class="" v-html="d.status_description ? d.status_description : ''"></td>
            <td class="" v-if="d.status_id==1"><small class="text-primary bi bi-check-square"></small></td>
            <td class="" v-else><span class="text-danger bi bi-x-square"></span></td>
...
resetForm: function(){
   ...
   validateForm: function(){
    ...
    sendPost: function(){
        ...
        axios.post('/statuses/create', form).then(response => {
            this.button=this.btntxt;
    ...
    onSearch: function(){
       ...
       for (let i = 0; i < this.totalRecord.length; i++) {
           if (
            this.totalRecord[i].status_name.toString().toLowerCase().search(this.search.toString().toLowerCase())!=-1 ||
    ...
    getRecords: function(){
        ...
        this.showOverlay=true;
        this.search = ''
        axios.get('/statuses/list', {params:this.parameters}).then(response => {
            this.button=this.btntxt;
            this.showOverlay=false;
            this.errors = '';
            if((response.status != undefined && response.status==200) && (response['data'].data.status==response['data'].data.statusmsg)){
    ...
    manageStatus: function(){
        ...
        $(".toaster").toast('hide')
        this.showOverlay=true;
        axios.get('/statuses/status/updatebatch', {params:this.parameters}).then(response => {
            this.button=this.btntxt;
            this.showOverlay=false;
            this.errors = '';
             if((response.status != undefined && response.status==200) && (response['data'].data.status==response['data'].data.statusmsg)){
