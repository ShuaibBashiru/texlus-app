...
<div class="">
  ...
  <div class="col-md-12">
        <div class="border rounded-3 p-1 pt-0 pb-0">
          <div class="row m-0 p-0 pb-1 pt-1">
            ...
            <input type="date" id="filter" title="Filter" v-model="selectedDate" class="btn float-end mt-2 mb-2 me-2 pt-1 pb-1 btn-outline-primary" @input="getRecords">
        </div>
      </div>
      </div>
  </div>
 ...
 <div class="row bg-primary text-white rounded pt-3 pb-3 m-0 mt-2">
      ...
      <div class="row overflow-hidden m-0 mt-2 mb-2">
        <div class="col-md-12">
          ...
          <small class="">Invoiced on (<small v-text="selectedDate"></small>)</small> 
              <hr> 
          <h5 class="mt-4 mb-0"> <span class="" v-text="valueConverter(totalMonthlyAmount)"></span> </h5>
          <small class="" v-text="'Revenue in '+months[parameters.month]"></small> 
          <hr>
          <h5 class="mt-4 mb-0"> <span class="" v-text="totalMonthlyInvoiced"></span> </h5>
          <small class="" v-text="'Invoiced in '+months[parameters.month]"></small>
    </div>
    </div>
    </div>
    </div>
  ...
  <div class="col-md-9 mt-2">
      ...
      <div class="row border-bottom m-0 p-0 pb-1 pt-1">
              <div class="col-md-8">
            <h6 class="m-0 pt-2 pb-2" v-text="chartOptions.optionFLow.header + ' ' +months[parameters.month]+ ', '+parameters.year"></h6> 
          </div>
              <div class="col-md-4">
              </div>
    </div>
 ...
 getRecords: function(){
  ...
  filterData: function(needle, record){
    ...
    this.chartOptions.optionFLow.title = "";
          ...
          var total = 0;
          var qty = 0;
          var selectedDate = columns[i].split('-')
          ...
          for (let index = 0; index < grouped[columns[i]].length; index++) {
              ...
              if (this.selectedDate == columns[i]) {
                countTodayInvoice += 1
                todayAmount += parseFloat(grouped[columns[i]][index].totalPrice);
                countMonthlyInvoice += 1
                MonthlyAmount += parseFloat(grouped[columns[i]][index].totalPrice);
              }else if(this.parameters.month == selectedMonth){
                countMonthlyInvoice += 1
                MonthlyAmount += parseFloat(grouped[columns[i]][index].totalPrice);
              }
          }
          var label = this.selectedDate == columns[i]? columns[i].toString()+'\nActive' : columns[i].toString()
          this.chartFLows.push([label, total])
          }
          this.totalPrice = todayAmount;
          this.totalItems = countTodayInvoice;
          this.totalMonthlyAmount = MonthlyAmount;
